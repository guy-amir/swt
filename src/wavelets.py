...
class wavelet():
    ...
    def construct_mu_nodes(self):        
        ...
        mu_mean = torch.mean(torch.cat(self.tree.nu),0).unsqueeze(1) ##GG add last tensor in mu_cache list later
    ...
    def cutoff(self,N):
        #get ordered wavelet list
        self.get_psi()
        #cutoff after N+1 (or N) values: 
        node_list = self.psi.Norm_order[:N]
